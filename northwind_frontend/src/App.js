...
class App extends Component {
  ...
  render() {
    ...
    const listItems = items.map((item) =>
      <li key={item.idProvedor} onClick={() => this.onSelect(item.getProvedor)}>
         <span className="item-name">{item.nombreCompania}</span> |  {item.paginaProvedor}
      </li>
    );
    return (
      <div className="App">
          ...
          <br/>
          <button type="button" name="button" onClick={() => this.onNewItem()}>Nuevo Provedor</button>
          <br/>
  ...
  onSelect(idProvedor) {
    this.clearState();
    this.itemService.getItem(idProvedor).then(item => {
      this.setState({
          showDetails: true,
          selectedItem: item
        });
  ...
  onDeleteItem(idProvedor) {
    this.clearState();
    this.itemService.deleteItem(idProvedor).then(res => {
        this.getItems();
      }
    );
  }
