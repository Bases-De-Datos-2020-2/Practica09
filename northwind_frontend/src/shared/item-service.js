...
class ItemService {
  ...
  return fetch(this.config.ITEM_COLLECTION_URL)
    ...
    .then(json => {
      ...
      for(var i = 0; i < itemArray.length; i++) {
        itemArray[i]["getProvedor"] = this.config.ITEM_COLLECTION_URL + '/' +itemArray[i].idProvedor;
  ...
  async updateItem(item) {
    ...
    console.log(item);
    return fetch(this.config.ITEM_COLLECTION_URL+"/" + item.idProvedor, {
      method: "PUT",
      mode: "cors",
      headers: {
            "Content-Type": "application/json"
